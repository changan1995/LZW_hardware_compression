...
int main()
{
    ...
    while (offset < InputLen - 1)
    {
        ...
        uint64_t tmpir[256], tmpout[256];
        uint32_t chunk_size_sw[CHUNK_NUMBER] = {0};
        // uint32_t chunk_size_hw[CHUNK_NUMBER] = {0};
        uint32_t offsets[CHUNK_NUMBER] = {0};
        offsets[0] = offset;
        for (int i = 0; i < CHUNK_NUMBER; i++)
        {
            memcpy(tmpir, ir, sizeof(uint64_t) * 256);
            memcpy(tmpout, out, sizeof(uint64_t) * 256);
            chunk_size_sw[i] = Content_Defined_Chunk_sw(buf + offset, InputLen - offset, ir, out);
#ifdef __SDSCC__
            t[9] += sds_clock_counter() - start;
            start = sds_clock_counter();
#endif
            // chunk_size_hw[i] = Content_Defined_Chunk_hw(buf + offset, InputLen - offset, tmpir, tmpout);
            // //assert(chunk_size_hw == chunk_size_sw);
            // if (chunk_size_hw != chunk_size_sw)
            // {
            //     error = 1;
            //     printf("Error: CDC HW version produced incorrect value!\n");
            //     printf("chunk_size_sw: %lu; chunk_size_hw: %lu\n", chunk_size_sw, chunk_size_hw);
            // }
#ifdef __SDSCC__
            ...
            start = sds_clock_counter();
#endif
            offsets[i + 1] = offsets[i] + chunk_size_sw[i];
        }
        ...
        for (int i = 0; i < CHUNK_NUMBER; i++)
        {
            sha256_hw(buf + offset, chunk_size_sw[i], hash_hw);
            id[i] = lookup(hash_hw);
        }
#ifdef __SDSCC__
        ...
        for (int i = 0; i < CHUNK_NUMBER; i++)
        {
            if (id[i] < 0)
            {
                uint32_t lzwInputLen = lzw_encode(buf + offset, chunk_size_sw[i], outbuf + outoffset + 4);
            ...
            {
                uint32_t temp = id[i] << 1 | 1;
                memcpy(outbuf + outoffset, &temp, sizeof(uint32_t));
                outoffset += 4;
                duplicate_size += chunk_size_sw[i];
            }
#ifdef __SDSCC__
...
#endif
            header_size += 4;
            offset += chunk_size_sw[i];
        }
    }
#ifdef __SDSCC__
    printf("Execution time for CDC_sw: %llu cycles; throughput: %f\n", t[9], getTp(InputLen, t[9]));
    printf("Execution time for CDC_hw: %llu cycles; throughput: %f\n", t[0], getTp(InputLen, t[0]));
