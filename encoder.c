...
int main()
{
	...
	while(offset<InputLen-1)
	{
        ...
        for(int i =0; i<BUCKET_SIZE;i++) {
            *lzw_output_buffer_bucket = alloc(CHUNK_SIZE); // initialize buffer
            uint32_t chunk_size_temp = Content_Defined_Chunk(buf, offset_bucket[i], InputLen, ir, out);
            offset_bucket[i+1] = offset_bucket[i] + chunk_size_temp;
        }
        ...
        for(int i =0; i<BUCKET_SIZE;i++) {
            sha256_hw(buf+offset_bucket[i], chunk_size_bucket[i], hash_hw);
                    ...
                    {   
                        ...
                        uint32_t lzwInputLen = lzw_encode(buf+offset, chunk_size_bucket[i], lzw_output_buffer_bucket[i]);
                        uint32_t temp = lzwInputLen << 1;
                        memcpy(outbuf + outoffset, &temp, sizeof(uint32_t));
                        memcpy(outbuf + 4, lzw_output_buffer_bucket[i], lzwInputLen);
